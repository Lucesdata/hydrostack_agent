...
describe("cuantiaGate (L0 — 3 bandas, D5)", () => {
  it("valor dentro del rango objetivo → PASS", () => {
    expect(
      cuantiaGate(profile, proc({ precioBase: 500_000_000 }), DEFAULT_CUANTIA_BANDS).status
    ).toBe("PASS");
  });
  ...
  it("valor en el margen ±20% del borde → WARN", () => {
    ...
    expect(
      cuantiaGate(profile, proc({ precioBase: 1_150_000_000 }), DEFAULT_CUANTIA_BANDS).status
    ).toBe("WARN");
  });

  it("valor muy fuera del rango → FAIL", () => {
    expect(
      cuantiaGate(profile, proc({ precioBase: 5_000_000_000 }), DEFAULT_CUANTIA_BANDS).status
    ).toBe("FAIL");
  });
  ...
  it("proceso sin valor en metadata → UNKNOWN", () => {
    expect(
      cuantiaGate(
        profile,
        proc({ precioBase: null, valorAdjudicacion: null }),
        DEFAULT_CUANTIA_BANDS
      ).status
    ).toBe("UNKNOWN");
  });
...
describe("plazoGate (L0 parcial, D1)", () => {
  ...
  it("sin apertura ni fecha → UNKNOWN", () => {
    expect(plazoGate(proc({ estadoApertura: null, fechaCierre: null }), NOW).status).toBe(
      "UNKNOWN"
    );
  });
...
describe("ubicacionGate (L0 — DIVIPOLA depto)", () => {
  ...
  it("departamento no reconocido → UNKNOWN", () => {
    expect(ubicacionGate(profile, proc({ departamento: "Tierra del Nunca" })).status).toBe(
      "UNKNOWN"
    );
  });
  ...
  it("#2 cobertura solo por municipio (departamentos:[]) → PASS si el municipio coincide", () => {
    ...
    expect(
      ubicacionGate(soloMuni, proc({ departamento: "Valle del Cauca", ciudad: "Cali" })).status
    ).toBe("PASS");
  });
...
describe("aggregateVerdict (D6 — worst-of, UNKNOWN aparte)", () => {
  const g = (status: "PASS" | "WARN" | "FAIL" | "UNKNOWN") => ({
    status,
    reason: "",
    resolvedBy: "metadata" as const,
    requiredLevel: 0 as const,
  });
...
describe("buildVerdict (orquestador)", () => {
  it("compone las 5 compuertas, overall = agregación, level 0", () => {
    ...
    expect(Object.keys(v.gates).sort()).toEqual([
      "cuantia",
      "habilitacion",
      "plazo",
      "sectorial",
...
describe("habilitacionGate (L2 — con requisitos estructurados)", () => {
    ...
    const p = proc({
      requisitosHabilitantes: {
        experiencia: {
          ...
          cita_textual: "remite al anexo 3",
        },
        indicadores_financieros: [],
      },
    });
  ...
  it("max_contratos_aportables: el mejor subconjunto de tamaño N SÍ alcanza el umbral → PASS (no descalifica por exceso de contratos)", () => {
    ...
    const perfilConVariosContratos: OferenteProfile = {
      ...
      experiencia: [
        {
          objeto: "PTAP municipal A",
          valorSmmlv: 600,
          unspscCodigos: ["83101500"],
          anioTerminacion: 2021,
        },
        {
          objeto: "PTAP municipal B",
          valorSmmlv: 500,
          unspscCodigos: ["83101500"],
          anioTerminacion: 2022,
        },
        {
          objeto: "PTAP municipal C",
          valorSmmlv: 50,
          unspscCodigos: ["83101500"],
          anioTerminacion: 2023,
        },
