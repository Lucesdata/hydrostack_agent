...
function row(over: Record<string, unknown>) {
  return {
    [F.fase]: "",
    [F.estado]: "",
    [F.modalidad]: "Licitación pública",
    [F.url]: SECOP_II,
    ...over,
...
describe("accessMessage", () => {
  it("da un mensaje no vacío y distinto por estado", () => {
    const msgs = (["PUBLIC", "RESTRICTED", "NOT_PUBLISHED", "UNKNOWN"] as DocumentAccess[]).map(
      accessMessage
    );
...
describe("classifyProbeResponse (C1)", () => {
  it("muro ReCaptcha por URL final tras redirect → RESTRICTED (caso real SECOP II)", () => {
    const r = classifyProbeResponse({
      ...
      status: 200,
      finalUrl:
        "https://community.secop.gov.co/Public/Common/GoogleReCaptcha/Index?previousUrl=...",
...
describe("probeDocument (C1, fetch inyectado)", () => {
  ...
  function fakeRes(over: {
    ...
    return {
      ...
      url: over.url,
      headers: {
        get: (k: string) => (k.toLowerCase() === "content-type" ? over.contentType : null),
      },
