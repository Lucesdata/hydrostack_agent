...
describe("runIngest", () => {
  ...
  it("stops on the first empty page", async () => {
    ...
    const summary = await runIngest(
      { fetchPage, sink: countingSink },
      {
        source: SOURCE_CONTRATOS,
        lastWatermark: "2024-06-01T00:00:00.000",
        batchId: BATCH,
        pageSize: 100,
      }
    );
  ...
  it("applies the overlap window (D14) on the first page", async () => {
    ...
    await runIngest(
      ...
      {
        source: SOURCE_CONTRATOS,
        lastWatermark: "2024-06-04T00:00:00.000",
        batchId: BATCH,
        marginDays: 1,
      }
    );
  ...
  it("stops at maxPages and flags the truncation", async () => {
    let n = 0;
    const fetchPage: SodaFetcher = async () => [
      row(`id${n}`, `2024-01-${String((n++ % 28) + 1).padStart(2, "0")}T00:00:00.000`),
    ];
    const summary = await runIngest(
      { fetchPage, sink: countingSink },
