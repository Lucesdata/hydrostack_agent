export type Route = "normativa" | "dimensionado" | "suelo" | "materiales" | "mantenimiento";
...
export const CATALOG: CatalogQuestion[] = [
  // ─── RUTA: NORMATIVA ──────────────────────────────────────────────────────
  {
    ...
    intent:
      "Explicar qué norma se está usando (RAS/CTE/EN 12566/EPA) según la selección del usuario. Mencionar que la norma define requisitos mínimos para T_r (tiempo de retención), separaciones a pozos de agua, volumen mínimo, y dimensiones.",
...

  {
    ...
    intent:
      "Explicar que cada norma define un volumen mínimo basado en su experiencia regional. RAS: 1.0 m³, España: 1.5 m³, Europa: 2.0 m³, EPA: 3.785 m³. Esto refleja criterios de durabilidad, acceso para mantenimiento y desempeño digestión anaerobia en climas distintos.",
    references: [
      "RAS Título J 2017 §E.4.1.4",
      "CTE DB-HS5 §4.3.2",
      "EN 12566-1 §3",
      "EPA 625/R-06/003",
    ],
...

  {
    ...
    intent:
      "Indicar la separación mínima según norma: RAS establece 30 m a pozos de agua, EPA establece 15 m. Mencionar que esta separación protege la calidad del agua subterránea ante posibles infiltraciones de efluente parcialmente tratado.",
...

  {
    ...
    conditions: ["form_calculated"],
    intent:
      "Mostrar que el T_r varía con temperatura porque los microorganismos anaerobios digieren materia orgánica más rápido a mayor T. Ejemplos: RAS a ≥20°C: 1.5d, a <10°C: 2.5d. Clima frío requiere más días para digestión completa. Informar T_r aplicado: {r.p.trhDays} días ({r.p.tempLabel}).",
...

  {
    ...
    conditions: ["form_calculated"],
    intent:
      "Indicar que la norma aplicada ({norm_name}) define mínimos para permitir sedimentación, evitar cortocircuitos, y facilitar acceso. Mostrar: minWidth, minLength, minDepth según norma. Validar que calculado cumple: L={r.L}m ≥ {r.p.minLength}m, W={r.W}m ≥ {r.p.minWidth}m, depth={r.depth}m ≥ {r.p.minDepth}m.",
...

  {
    ...
    conditions: ["form_calculated"],
    intent:
      "Explicar que CVO = (DBO₅ influente × caudal) / Vtot, expresada en kg/m³·día. Límite máximo: 0.30 kg/m³·día. Por encima, la fosa se sobrecarga y la digestión anaerobia colapsa. Resultado actual: CVO={r.CVO} kg/m³·día, estado={r.chkCVO?'✓ Aceptable':'✗ CRÍTICO'}.",
  ...
  // ─── RUTA: DIMENSIONADO ──────────────────────────────────────────────────
  {
    ...
    conditions: ["form_calculated"],
    intent:
      "Desglosar el volumen total: Vl (líquido) = caudal × T_r, Vs (lodos) = usuarios × tasa_acumulación × años, Vn (natas) = factor × Vl. Mostrar los tres componentes con valores: Vl={r.Vl}m³, Vs={r.Vs}m³, Vn={r.Vn}m³, Vtot={r.Vtot}m³. Si {r.minA}, indicar que se aplicó el mínimo normativo ({r.p.minVolume}m³).",
...

  {
    ...
    intent:
      "Explicar que aumentar usuarios incrementa: (1) caudal diario Q_d, por tanto Vl aumenta; (2) producción de lodos Vs aumenta. Ambos impactan Vtot directamente. También puede cambiar número de cámaras (1→2→3 según volumen). Ej: si usuarios={users}, Qd={r.Qd}m³/día.",
...

  {
    ...
    conditions: ["form_calculated"],
    intent:
      "Para vivienda doméstica (tipo_uso='Doméstico'), dar rangos según número de usuarios: 1-3 hab: 1.5-3.0 m³; 4-6 hab: 2.5-4.5 m³; 7-10 hab: 4.0-7.0 m³. Comentar que el resultado actual ({r.Vtot}m³) está en rango típico o fuera. Referencia: este cálculo sigue {norm_name}.",
...

  {
    ...
    conditions: ["form_calculated"],
    intent:
      "Significa que el volumen calculado (Vl+Vs+Vn) fue menor que el volumen mínimo exigido por la norma. Calculadora automáticamente usa el mínimo. Ej: calculado {r.Vtot_calc}m³, pero norma exige mín {r.p.minVolume}m³, por tanto Vtot={r.p.minVolume}m³. Esto garantiza desempeño mínimo asegurado.",
...

  {
    ...
    conditions: ["form_partial", "form_calculated"],
    intent:
      "El período de limpieza define cuántos años se acumulan lodos sin vaciar. Vs = usuarios × tasa_acumulación × años_limpieza. Si cambio periodo de {cleanYears} a X años, Vs varía proporcionalmente. Períodos más largos → mayor Vtot necesario. Período actual: {cleanYears} años.",
...

  {
    ...
    conditions: ["form_calculated"],
    intent:
      "Sí, muy sensible. Temperatura afecta T_r (varía 1.0–4.0 días según norma y clima). Menor T_r → menor Vl → menor Vtot. En climas fríos (T<10°C) el T_r aumenta significativamente. Ajuste de temperatura de {temp}°C a X°C cambia T_r de {r.p.trhDays}d a otro valor y recalcula todo. Efecto en Vtot es directo.",
...

  {
    ...
    conditions: ["form_calculated"],
    intent:
      "El número de cámaras mejora sedimentación y retención. Regla: usuarios >50 o Vtot >10 m³ → 3 cámaras; usuarios >5 o Vtot >2 m³ → 2 cámaras; sino → 1 cámara. Actual: {r.chambers} cámaras. Esto permite mayor contacto del efluente con biomasa anaeróbica.",
  ...
  // ─── RUTA: SUELO ─────────────────────────────────────────────────────────
  {
    ...
    conditions: ["form_partial", "form_calculated"],
    intent:
      "Verificar que suelo seleccionado sea apto (ok=true). Si suelo seleccionado es 'Arcilla (no apto)' o similar, responder NO: arcilla pura no infiltra; es necesaria alternativa. Si es apto, responder SÍ con la tasa q estimada. Tipo seleccionado: {soil_label}, q={soil.q} L/m²·día, apto={soil.ok}.",
...

  {
    ...
    intent:
      "Si usa tabla predefinida, q está asociado a tipo de suelo. Si selecciona 'Manual (test perc.)', debe ingresar tiempo de percolación (min/cm) del ensayo in situ. La fórmula es aprox: q ≈ 70/√T en L/m²·día. El test de percolación es el único método preciso; la tabla es orientativa. Actual: {soil_label}.",
...

  {
    ...
    conditions: ["form_calculated"],
    intent:
      "Área = caudal diario / q del suelo. A_inf = {r.Qd*1000} L/día ÷ {soil.q} L/m²·día = X m². Campo debe ser al menos esta área, preferiblemente 20–30% mayor para seguridad. Profundidad típica 0.6–1.0 m. Si q es muy baja (suelo limo/arcilla), el campo puede resultar muy grande → considerar alternativas.",
...

  {
    ...
    conditions: ["form_calculated", "always"],
    intent:
      "Hormigón: más resistente mecánicamente, vida útil 40+ años, requiere mantenimiento periódico de grietas. PEAD: ligero, resistente a raíces, vida útil 30+ años, costo similar o menor. En zonas con agua freática alta, PEAD evita problemas de flotación. Elección depende de suelo, disponibilidad local y presupuesto. Ambos cumplen normativa.",
...

  {
    ...
    conditions: ["form_calculated"],
    intent:
      "Calculadora propone Ø {r.dPipe*1000:.0f} mm calculado desde caudal máximo {r.Qd*1000/86400:.2f} L/s con mínimo de 100 mm. Entrada debe estar sumergida ≥30 cm bajo superficie del agua; salida ≥40 cm bajo la más profunda. Tubería debe tener pendiente mínima 1–2% para evitar depósitos y zonas de turbulencia.",
...

  {
    ...
    conditions: ["form_calculated", "always"],
    intent:
      "Signos de fallo: (1) olores muy fuertes (H₂S sin diluir) → anaerobia desequilibrada; (2) efluente turbio/marrón → no hay sedimentación; (3) natas flotantes excesivas o sumergidas → imbalance C/N; (4) agua encharcada alrededor → infiltración fallida; (5) tuberías con depósitos visibles → bajapendiente o cortocircuitos. Cada signo requiere diagnóstico diferente.",
...

  {
    ...
    conditions: ["form_calculated", "always"],
    intent:
      "Inspección recomendada: Fosa séptica: 1–2 veces/año (mínimo anual después de primer año). Campo de infiltración: anual, observando si agua se encharca, olores, vegetación muerta. Registros de inspección deben documentarse para mantener garantía y trazabilidad. Primera inspección: 6 meses después de puesta en marcha.",
...

  {
    ...
    conditions: ["form_calculated", "always"],
    intent:
      "Prohibido: (1) Grasas/aceites de cocina (se solidifican y colmatan); (2) Papel no biodegradable, plásticos, textiles; (3) Químicos agresivos (lejía, desengrasantes industriales) → matan biomasa anaeróbica; (4) Medicamentos en cantidad (alteran microbiota); (5) Residuos sólidos. Solo: agua residual doméstica (WC, ducha, cocina sin grasa). Nunca usar selladores químicos que afirman 'limpiar' fosa → causan desequilibrio biológico.",
