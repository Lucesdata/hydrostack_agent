...
function asNumberOrNull(v: unknown, field: string): number | null {
  ...
  if (typeof v !== "number" || Number.isNaN(v))
    throw new Error(`campo ${field} debe ser número o null`);
  return v;
}
...
export function parseRequisitosEstructurados(raw: unknown): RequisitosHabilitantesEstructurados {
  if (typeof raw !== "object" || raw === null)
    throw new Error("requisitos estructurados: no es un objeto");
  ...
  if (!Array.isArray(e.unspsc_exigidos))
    throw new Error("experiencia.unspsc_exigidos debe ser un array");
  const experiencia: ExperienciaExigida = {
    valor_min_smmlv: asNumberOrNull(e.valor_min_smmlv, "experiencia.valor_min_smmlv"),
    unspsc_exigidos: e.unspsc_exigidos.map((c, i) =>
      asString(c, `experiencia.unspsc_exigidos[${i}]`)
    ),
    max_contratos_aportables: asNumberOrNull(
      e.max_contratos_aportables,
      "experiencia.max_contratos_aportables"
    ),
  ...
  const indicadores_financieros: IndicadorFinancieroExigido[] = o.indicadores_financieros.map(
    (it, i) => {
      if (typeof it !== "object" || it === null)
        throw new Error(`indicadores_financieros[${i}] inválido`);
      ...
      return {
        ...
        verificar_manual: asBoolean(
          ii.verificar_manual,
          `indicadores_financieros[${i}].verificar_manual`
        ),
