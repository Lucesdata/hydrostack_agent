...
export function buildExtractionPrompt(): string {
  return [
    ...
    '7. Cada ítem lleva `codigo`: el código o numeración exacta tal como aparece en el pliego/formato para ese ítem (p. ej. "1.1.73.1.1"); si el pliego no numera ítems, usa "NO_ENCONTRADO". También lleva `cita_textual`: la cita mínima (máx. ~20 palabras) del documento que sustenta ese ítem. Sin cita verificable, el ítem no se incluye.',
    '',
    ...
    '',
    '# LAGUNAS Y CONTRADICCIONES DEL PLIEGO (`lagunas_pendientes`)',
    'Identifica y reporta en `lagunas_pendientes` cualquier inconsistencia, ambigüedad o vacío',
    'del propio pliego que un proponente necesitaría resolver antes de ofertar: numeraciones',
    'distintas para el mismo ítem entre secciones, tablas que no cuadran entre sí, anexos',
    'referenciados en el texto pero no incluidos entre los archivos analizados, texto residual',
    'de una plantilla de otro tipo de contrato, o cualquier otra contradicción interna del',
    'documento. Cada laguna lleva: `id` (entero secuencial desde 1), `descripcion` (qué es la',
    'laguna y dónde se encuentra, citando página/sección si es posible), `severidad` ("alta",',
    '"media" o "baja" según el riesgo de rechazo o error para el proponente) y `tipo`',
    '(categoría breve, p. ej. "inconsistencia interna del pliego", "archivo externo no',
    'analizado", "residuo de template no depurado"). Si no detectas ninguna laguna, deja el',
    'array vacío — no inventes contradicciones que no existen.',
    '',
