...
export const SMART_COLLECTIONS: SmartCollection[] = [
  {
    ...
    pills: [
      {
        id: "coleccion:abiertos-sin-adjudicar",
        type: "coleccion",
        label: "Abierto · sin adjudicar",
        value: "abiertos-sin-adjudicar",
      },
    ],
  },
  {
    ...
    glyph: "💰",
    pills: [
      { id: "coleccion:alto-valor", type: "coleccion", label: "Alto valor", value: "alto-valor" },
    ],
  },
  {
    ...
    pills: [
      {
        id: "coleccion:listos-para-extraer",
        type: "coleccion",
        label: "Pliego público",
        value: "listos-para-extraer",
      },
    ],
  },
];
...
export function parseSearchQuery(raw: string): FilterPill[] {
  ...
  if (remainder) {
    pills.push({
      id: `texto:${remainder.toLowerCase()}`,
      type: "texto",
      label: `"${remainder}"`,
      value: remainder,
    });
...
export function matchesResidualPill(item: SecopProceso, pill: FilterPill): boolean {
  switch (pill.type) {
    ...
    case "coleccion":
      ...
      if (pill.value === "abiertos-sin-adjudicar")
        return item.estadoApertura === "Abierto" && !item.adjudicado;
