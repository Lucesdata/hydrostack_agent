  ...
  return (
    <g>
      ...
      <text x={x0+tW/2} y={(yWtr+yNatB)/2+3} fill="#9a8040" fontSize="8.5"
        textAnchor="middle" fontFamily={FONT}>NATAS — Vn = {F(r.Vn)} m³</text>
      <text x={x0+tW/2} y={(yNatB+yLiqB)/2+3} fill="#3a8090" fontSize="8.5"
        textAnchor="middle" fontFamily={FONT}>ZONA LÍQUIDA — Vl = {F(r.Vl)} m³</text>
      {ySldB-yLiqB>16&&(
        <text x={x0+tW/2} y={(yLiqB+ySldB)/2+3} fill="#8a5a28" fontSize="8.5"
          textAnchor="middle" fontFamily={FONT}>LODOS — Vs = {F(r.Vs)} m³</text>
      )}
      {/* Dividers */}
      {divs.map((dx,i)=>(
...
function SitardScheme({ r, x, y, w, h }) {
  ...
  const items=[
    {lbl:"VIVIENDA",     sub:`${r.users||"—"} hab`,       det:`Q=${F(r.Qd||0)} m³/d`, col:DS.green, bg:"rgba(0,255,136,0.07)", icon:"🏠"},
    {lbl:"TRAMPA GRASAS",sub:"③ L+G",                    det:"retención",             col:"#8ab020", bg:"rgba(130,180,20,0.07)",icon:"🧱"},
    {lbl:"FOSA SÉPTICA", sub:`Vtot=${F(r.Vtot)} m³`,     det:`${r.chambers} cámara(s)`,col:DS.cyan,  bg:"rgba(0,245,255,0.07)",icon:"⬛"},
    {lbl:"FAFA/FILTRO",  sub:"⑥ anaerobio",             det:"lecho bacteriano",      col:"#A78BFA", bg:"rgba(167,139,250,0.07)",icon:"🔲"},
    {lbl:"CAJA INSP.",   sub:"⑦ registro",              det:"rev. periódica",        col:DS.amber,  bg:"rgba(255,176,32,0.07)", icon:"⬜"},
    {lbl:"INFILTRACIÓN", sub:r.A_inf?`A≈${F(r.A_inf)}m²`:"ver estudio",det:"campo ⑧",col:DS.green, bg:"rgba(0,255,136,0.07)", icon:"🌱"},
  ];
  const segW=cW/items.length, boxW=Math.min(segW*0.72,118), boxH=66;

  return (
    <g>
      <SecTitle x={x} y={y} w={w} num="4" label="ESQUEMA SITARD — SISTEMA INTEGRAL DE TRATAMIENTO"/>
      {/* Ground */}
      ...
      <text x={x0+cW/2} y={y0+cH-7} fill={DS.muted} fontSize="8.5"
        ...
        DBO₅: {r.dboIn||250} mg/L  →  ≈{F(r.dboOut||0)} mg/L salida  |  SS: {r.ssIn||280} mg/L  →  ≈{F(r.ssOut||0)} mg/L  |  Remoción ≈{r.chambers===1?"60":"68"}%
      </text>
    </g>
  );
}
...
function DataTable({ r, x, y, w, h }) {
  ...
  const legend=[
    ["①","Tubería entrada (T-pipe)"],["②","Tubería salida (T-pipe)"],
    ["③","Trampa de grasas"],        ["④","Fosa séptica"],
  ...
  return (
    <g>
      ...
      <text x={px+2} y={py+10} fill={DS.muted} fontSize="8" fontFamily={FONT}>PARÁMETRO</text>
  ...
  return (
    <g>
      ...
      <text x={W*0.14} y={y0+44} fill="rgba(74,122,138,0.6)" fontSize="8"
        textAnchor="middle" fontFamily={FONT}>FOSA SÉPTICA — Plano HID-01</text>
      {/* Fields */}
      {[...row1,...row2].map((f,i)=>(
        <g key={i}>
