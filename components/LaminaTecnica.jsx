  ...
  return (
    <g>
      ...
      <text
        ...
        textAnchor="middle"
        fontFamily={FONT}
        transform={`rotate(-90,${x1 + dx - 4},${my + 3})`}
      >
        {lbl}
      </text>
    </g>
...
const SecTitle = ({ x, y, w, num, label }) => (
  <g>
    ...
    <text x={x + 30} y={y + 12} fill={DS.muted} fontSize="8" fontFamily={FONT} letterSpacing="1.5">
      {label}
    </text>
  </g>
);
...
function PlantView({ r, x, y, w, h }) {
  ...
  const tW = r.L * scl,
    tH = r.W * scl;
  const ox = x + (w - tW) / 2,
    oy = y + TH + PAD + (aH - tH) / 2 + 8;
  const wt = 9;

  const divs =
    r.chambers === 3 ? [ox + tW * 0.5, ox + tW * 0.75] : r.chambers === 2 ? [ox + tW * 0.667] : [];

  const hatches =
    r.chambers === 1
      ? [ox + tW / 2]
      : r.chambers === 2
        ? [ox + tW * 0.333, ox + tW * 0.833]
  ...
  const chamLbls =
    r.chambers === 1
      ...
      : r.chambers === 2
        ? [
            { cx: ox + tW * 0.333, l: "C-1" },
            { cx: ox + tW * 0.833, l: "C-2" },
          ]
        : [
            { cx: ox + tW * 0.25, l: "C-1" },
            { cx: ox + tW * 0.625, l: "C-2" },
            { cx: ox + tW * 0.875, l: "C-3" },
          ];
  ...
  return (
    <g>
      ...
      <text x={hatches[0] + 10} y={oy + wt + 13} fill={DS.green} fontSize="7.5" fontFamily={FONT}>
        ⑤
      </text>
      {/* Inlet */}
      <rect
      ...
      <g transform={`translate(${ox + tW + 32},${oy + 18})`}>
        ...
        <text x={0} y={22} fill={DS.muted} fontSize="7" textAnchor="middle" fontFamily={FONT}>
          N
        </text>
      </g>
      {/* Dims */}
...
function CrossSection({ r, freeboard, x, y, w, h }) {
  ...
  const hNata = r.Vn / Area,
    hLiq = r.Vl / Area,
    hSlud = r.Vs / Area;
  const yWtr = y0 + freeboard * sy,
    yNatB = yWtr + hNata * sy;
  const yLiqB = yNatB + hLiq * sy,
    ySldB = yLiqB + hSlud * sy,
    yBot = y0 + tH;

  const divs =
    r.chambers === 3 ? [x0 + tW * 0.5, x0 + tW * 0.75] : r.chambers === 2 ? [x0 + tW * 0.667] : [];
  ...
  return (
    <g>
      ...
      <rect
        ...
        y={y0 + wt}
        width={tW - wt * 2}
        height={Math.max(yWtr - y0 - wt, 0)}
        fill="rgba(2,12,16,0.7)"
      />
      <text
        x={x0 + tW - wt - 5}
        y={y0 + wt + (yWtr - y0 - wt) / 2 + 3}
...
function HydraulicProfile({ r, x, y, w, h }) {
  ...
  const bars = [
    { lbl: "Entrada", sub: "Ks=0.5", v: r.hf_entrada },
    { lbl: "Contrac.", sub: "Kc=0.5", v: r.hf_contraccion },
    { lbl: "Fosa", sub: "H-W", v: r.hf_fosa },
    { lbl: "Tabiques", sub: "Kt=0.3", v: r.hf_tabiques },
    { lbl: "Salida", sub: "Ks=1.0", v: r.hf_salida },
  ];
  ...
  return (
    <g>
      ...
      <text
        ...
        textAnchor="middle"
        fontFamily={FONT}
        transform={`rotate(-90,${x0 - 13},${y0 + cH / 2})`}
      >
        cm
      </text>
      {/* Bars */}
      {bars.map((b, i) => {
        ...
        const bx = x0 + i * bGap + bGap / 2 - bW / 2,
          by = y0 + cH - bh;
        return (
          <g key={i}>
            <rect
...
function SitardScheme({ r, x, y, w, h }) {
  ...
  const items = [
    ...
    },
    {
      ...
      det: "retención",
      col: "#8ab020",
      bg: "rgba(130,180,20,0.07)",
      icon: "🧱",
    },
    {
      lbl: "FOSA SÉPTICA",
      sub: `Vtot=${F(r.Vtot)} m³`,
      det: `${r.chambers} cámara(s)`,
      col: DS.cyan,
    ...
    {
      lbl: "INFILTRACIÓN",
      sub: r.A_inf ? `A≈${F(r.A_inf)}m²` : "ver estudio",
      det: "campo ⑧",
      col: DS.green,
      bg: "rgba(0,255,136,0.07)",
      icon: "🌱",
  ...
  return (
    <g>
      <SecTitle
        ...
        w={w}
        num="4"
        label="ESQUEMA SITARD — SISTEMA INTEGRAL DE TRATAMIENTO"
      />
      {/* Ground */}
      <rect
        x={x0}
      ...
      {items.map((item, i) => {
        ...
        return (
          <g key={i}>
            ...
            />
            {/* Top accent */}
            <rect x={bx} y={by} width={boxW} height={5} rx="5" fill={item.col} opacity="0.4" />
            {/* Icon */}
            <text x={cx} y={by + 24} textAnchor="middle" fontSize="16" fontFamily="sans-serif">
              {item.icon}
            </text>
            {/* Label */}
            <text
      ...
      <text
        ...
        fontFamily={FONT}
      >
        DBO₅: {r.dboIn || 250} mg/L → ≈{F(r.dboOut || 0)} mg/L salida | SS: {r.ssIn || 280} mg/L → ≈
        {F(r.ssOut || 0)} mg/L | Remoción ≈{r.chambers === 1 ? "60" : "68"}%
      </text>
    </g>
  );
}
...
function DataTable({ r, x, y, w, h }) {
  ...
  const rows = [
    ...
    { lbl: "Hu — Prof. útil", val: `${F(r.depth)} m`, hi: true },
    { lbl: "BL — Borde libre", val: `${F(r.hT - r.depth, 2)} m`, hi: false },
    { lbl: "H — Prof. total", val: `${F(r.hT)} m`, hi: false },
    { lbl: "Vtot — Vol. diseño", val: `${F(r.Vtot)} m³`, hi: true },
    { lbl: "Vl — Vol. líquido", val: `${F(r.Vl)} m³`, hi: false },
    ...
    { lbl: "Ø Tub. ventilación", val: `${FI(r.dVent * 1000)} mm`, hi: false },
    { lbl: "N° Cámaras", val: `${r.chambers}`, hi: false },
    { lbl: "Hf hidráulica", val: `${(r.hf_total * 100).toFixed(1)} cm`, hi: false },
  ];

  const legend = [
    ["①", "Tubería entrada (T-pipe)"],
    ["②", "Tubería salida (T-pipe)"],
    ["③", "Trampa de grasas"],
    ["④", "Fosa séptica"],
    ["⑤", "Tubería ventilación"],
    ["⑥", "Filtro / FAFA"],
  ...
  return (
    <g>
      ...
      <text x={px + 2} y={py + 10} fill={DS.muted} fontSize="8" fontFamily={FONT}>
        PARÁMETRO
      </text>
      <text
        x={px + aW - 6}
      ...
      {rows.map((row, i) => (
        <g key={i}>
          ...
          <text
            ...
            fontSize="8"
            fontWeight={row.hi ? "bold" : "normal"}
            textAnchor="end"
            fontFamily={row.hi ? ORBITRON : FONT}
          >
            {row.val}
          </text>
        </g>
      ...
      {legend.map(([id, txt], i) => (
        <text
          ...
          fill="rgba(74,122,138,0.8)"
          fontSize="7.5"
          fontFamily={FONT}
        >
          <tspan fill={DS.cyan} fontWeight="bold">
            {id}
          </tspan>
          {"  "}
          {txt}
...
function Cajétin({ meta, W, H, cajeH }) {
  ...
  const row1 = [
    { lbl: "PROYECTO", val: meta.projectName || "—", x: W * 0.28 + 10, y: y0 + 16 },
    { lbl: "ELABORÓ", val: meta.designer || "—", x: W * 0.56 + 10, y: y0 + 16 },
    { lbl: "NORMA", val: meta.normRef || "RAS 2017 Tít. J", x: W * 0.73 + 10, y: y0 + 16 },
    { lbl: "PLANO N°", val: "HID-01", x: W * 0.87 + 10, y: y0 + 16 },
  ];
  const row2 = [
    { lbl: "UBICACIÓN", val: meta.location || "—", x: W * 0.28 + 10, y: y0 + cajeH / 2 + 16 },
    { lbl: "FECHA", val: today, x: W * 0.56 + 10, y: y0 + cajeH / 2 + 16 },
    { lbl: "ESCALA", val: "S / E", x: W * 0.73 + 10, y: y0 + cajeH / 2 + 16 },
  ...
  return (
    <g>
      ...
      />
      {/* Logo block */}
      <rect x={2} y={y0} width={W * 0.28 - 2} height={cajeH - 2} fill="rgba(0,245,255,0.04)" />
      <text
        x={W * 0.14}
        y={y0 + 20}
        fill={DS.cyan}
      ...
      <text
        x={W * 0.14}
        y={y0 + 44}
        fill="rgba(74,122,138,0.6)"
        fontSize="8"
        textAnchor="middle"
        fontFamily={FONT}
      >
        FOSA SÉPTICA — Plano HID-01
      </text>
      {/* Fields */}
      {[...row1, ...row2].map((f, i) => (
        <g key={i}>
          ...
          <text x={f.x} y={f.y + 10} fill={DS.text} fontSize="9" fontFamily={FONT}>
            {f.val}
          </text>
        </g>
      ))}
...
export default function LaminaTecnica({ r, freeboard, meta }) {
  const W = 1587,
    ...
    cajeH = 52;
  const drawH = H - cajeH;
  const topH = Math.round(drawH * 0.52),
    botH = drawH - topH;
  ...
  const z = {
    ...
    sitard: { x: 0, y: topH, w: Math.round(W * 0.6), h: botH },
    tabla: { x: Math.round(W * 0.6), y: topH, w: W - Math.round(W * 0.6), h: botH },
  };
  ...
  const exportSVG = () => {
    ...
    const a = Object.assign(document.createElement("a"), {
      href: url,
      download: `HID-01_FosaSeptica_${(meta.projectName || "Proyecto").replace(/\s+/g, "_")}.svg`,
    });
    a.click();
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "10px" }}>
        ...
        <span
          style={{ fontSize: "9px", color: "#4A7A8A", fontFamily: "'IBM Plex Mono',monospace" }}
        >
      ...
      {/* Drawing */}
      <div
        style={{ overflowX: "auto", border: "1px solid rgba(0,245,255,0.12)", borderRadius: "4px" }}
      >
        <svg
          id="hs-lamina"
          width={W}
