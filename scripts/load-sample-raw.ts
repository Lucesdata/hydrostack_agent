...
import { SOURCE_PROCESOS, SOURCE_CONTRATOS, type IngestSource } from "@/src/lib/ingest/sources";
...
async function main() {
  ...
  process.stdout.write(
    `procesos:  ${p.inserted} insertados · ${p.skipped} sin cambio (de ${p.total})\n`
  );

  const c = await loadSource(SOURCE_CONTRATOS, "contratos.json");
  process.stdout.write(
    `contratos: ${c.inserted} insertados · ${c.skipped} sin cambio (de ${c.total})\n`
  );
